...
export const threeDimensions: Sample = {
    ...
    draw: (handle: WebGPUHandle, geometry: GeometryData) => {
        ...
        if (handle.uniformBuffer) {
            const radius = 2;
            const speed = 0.033;
            const x = Math.sin(handle.frame * speed) * radius;
            const z = Math.cos(handle.frame * speed) * radius
            handle.camera.updateTransform(vec3.create(x, 0, z));
            const finalTransform = handle.camera.project(geometry.transform);
            handle.device.queue.writeBuffer(handle.uniformBuffer, 0, finalTransform.buffer);
        }
